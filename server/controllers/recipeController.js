...
exports.exploreRandom  =  async(req,res)=>{
  try {
  let count = await Recipe.findOneAndDelete().countDocuments();
...
exports.submitRecipe  =  async(req,res)=>{
  ...
  res.render('submit-recipe' ,{ title: "Cooking Blog - Recipe" ,infoErrorsObj , infoSubmitObj } );
  ...
  try {
    ...
    if(!req.files || Object.keys(req.files).length === 0){
     ...
     imageUploadFile.mv(uploadPath , function(err) {
      if(err) return res.statue(500).send(err);
     })
